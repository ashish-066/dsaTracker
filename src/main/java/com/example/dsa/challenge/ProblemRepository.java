...
public interface ProblemRepository extends JpaRepository<Problem, Long> {
    ...
    @Query("SELECT p FROM Problem p WHERE LOWER(p.difficulty) = LOWER(:diff) ORDER BY function('random')")
    List<Problem> findRandomByDifficulty(@Param("diff") String difficulty);

    /** Random problems for a specific platform and difficulty */
    @Query("SELECT p FROM Problem p WHERE LOWER(p.platform) = LOWER(:platform) AND LOWER(p.difficulty) = LOWER(:diff) ORDER BY function('random')")
    ...
    @Query("SELECT p FROM Problem p WHERE LOWER(p.platform) = LOWER(:platform) AND LOWER(p.topic) = LOWER(:topic) AND LOWER(p.difficulty) = LOWER(:diff) ORDER BY function('random')")
    ...
    @Query("SELECT p FROM Problem p WHERE LOWER(p.platform) IN :platforms AND LOWER(p.difficulty) = LOWER(:diff) ORDER BY function('random')")
    ...
    @Query("SELECT p FROM Problem p WHERE LOWER(p.platform) IN :platforms AND LOWER(p.topic) = LOWER(:topic) AND LOWER(p.difficulty) = LOWER(:diff) ORDER BY function('random')")
